...
impl Messenger for WasmMessenger {
    ...
    async fn offer_handshake(
        ...
        info!("[OFFER +0ms] start");

        let conn = create_rtc_peer_connection(ice_server_config);
        info!("[OFFER +{}ms] RTCPeerConnection created", js_sys::Date::now() - t0);
        ...
        info!("[OFFER +{}ms] create_offer done", js_sys::Date::now() - t0);
        ...
        JsFuture::from(conn.set_local_description(&rtc_session_desc_init_dict))
            ...
            .efix()
            .unwrap();
        info!("[OFFER +{}ms] set_local_description done", js_sys::Date::now() - t0);

        info!("[OFFER +{}ms] waiting for ICE gathering...", js_sys::Date::now() - t0);
        wait_for_ice_gathering_complete(conn.clone()).await;
        info!("[OFFER +{}ms] ICE gathering complete", js_sys::Date::now() - t0);

        signal_peer.send(PeerSignal::Offer(conn.local_description().unwrap().sdp()));
        info!("[OFFER +{}ms] offer sent to signaling server", js_sys::Date::now() - t0);

        let mut received_candidates = vec![];

        // Wait for answer
        info!("[OFFER +{}ms] waiting for answer...", js_sys::Date::now() - t0);
        let sdp = loop {
            ...
            match signal {
                ...
                PeerSignal::IceCandidate(candidate) => {
                    info!(
                        "[OFFER +{}ms] received ICE candidate while waiting for answer",
                        js_sys::Date::now() - t0
                    );
        ...
        info!("[OFFER +{}ms] answer received", js_sys::Date::now() - t0);
        ...
        JsFuture::from(conn.set_remote_description(&remote_description))
            ...
            .efix()
            .unwrap();
        info!("[OFFER +{}ms] remote description set", js_sys::Date::now() - t0);
        ...
        .await;
        info!("[OFFER +{}ms] handshake complete!", js_sys::Date::now() - t0);
    ...
    async fn accept_handshake(
        ...
        let t0 = js_sys::Date::now();
        info!("[ACCEPT +0ms] start, waiting for offer...");
        ...
        let offer = loop {
            ...
            match signal {
                ...
                PeerSignal::IceCandidate(candidate) => {
                    info!("[ACCEPT +{}ms] received ICE candidate before offer", js_sys::Date::now() - t0);
                    received_candidates.push(candidate);
                }
                _ => {
                    warn!("ignoring unexpected signal: {signal:?}");
        ...
        info!("[ACCEPT +{}ms] offer received", js_sys::Date::now() - t0);
        ...
        // Set remote description
        {
            ...
            info!("[ACCEPT +{}ms] remote description set", js_sys::Date::now() - t0);
        }

        let answer = JsFuture::from(conn.create_answer())
            .await
            .expect("error creating answer");
        info!("[ACCEPT +{}ms] answer created", js_sys::Date::now() - t0);
        ...
        JsFuture::from(conn.set_local_description(&session_desc_init))
            ...
            .efix()
            .unwrap();
        info!("[ACCEPT +{}ms] local description set, waiting for ICE gathering...", js_sys::Date::now() - t0);

        wait_for_ice_gathering_complete(conn.clone()).await;
        info!("[ACCEPT +{}ms] ICE gathering complete", js_sys::Date::now() - t0);

        let answer = PeerSignal::Answer(conn.local_description().unwrap().sdp());
        signal_peer.send(answer);
        info!("[ACCEPT +{}ms] answer sent", js_sys::Date::now() - t0);
        ...
        .await;
        info!("[ACCEPT +{}ms] handshake complete!", js_sys::Date::now() - t0);
...
fn create_rtc_peer_connection(ice_server_config: &RtcIceServerConfig) -> RtcPeerConnection {
    ...
    if ice_server_config.urls.is_empty() {
        // No ICE servers: host candidates only (fast for localhost)
        let empty: [IceServerConfig; 0] = [];
        peer_config.set_ice_servers(&serde_wasm_bindgen::to_value(&empty).unwrap());
    } else {
